...
def version_dic(version_root):
    ...
    for version_file in sorted(current_version_filelist):
        ...
        if not file_dic.get(version_shortfilename) or file_dic.get(version_shortfilename).get("version") < int(
                file_version):
...
update_path = os.path.join(r"D:/hda_log", time.strftime("%Y%m%d"),
                           "%s_%s.json" % ("new_otl", time.strftime("%Y%m%d_%H%M")))
